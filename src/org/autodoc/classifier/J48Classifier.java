...
* @author yogesh.gunge
*/
public class J48Classifier implements FeatureClassPredictor {
	...
	private boolean trainedModel = false;
	...
	public void saveModel() {
		...
		try {
			...
			SerializationHelper.write(outFilePath, classifier);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void loadModel() {
		...
		try {
			String outFilePath = Locations.dataPath + "/" + AutoDoc.mode + "/"
					+ AutoDoc.collection + "/"
					+ outputName + Settings.modelExt;
			classifier = (Classifier) SerializationHelper.read(outFilePath);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
    		...
    		while(trIter.hasNext()) {
    			...
    			try {
					...
					if(imageAllowed) {
				        ...
				        double[] classRes = classifier.distributionForInstance(i);
				        int classLabel = 0;
